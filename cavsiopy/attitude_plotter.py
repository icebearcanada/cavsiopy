...
def fov_plotter(extent, time_array, alt, lat, lon, fov_deg, Px, Py, 
                vec, vec_elev, inst_name, target_name, 
                step = 90, scale_arrow = 20, scale_factor_shift = 2.5):
    """
    Plot the FOV of the instrument.
    ...
    vec : numpy.ndarray[float]
        Instrument pointing vector in East-North-Center(ENC) coordinates.
    vec_elev : numpy.ndarray[float]
        Elevation angle (degrees).
    inst_name : string
        Name of the instrument.
    target_name : string
        Ground target to be captured.
    step : float, optional
        Time interval in seconds to plot the inst. vector. The default is 90.
    scale_arrow : float, optional
        Scaling of the inst. vector. The default is 20.
    scale_factor_shift : float, optional
        Scale factor to shift the ellipse center location. The default is 2.5.
    ...
    title ="".join(['Preliminary Plots\n', 'Start time:', 
                    time_array[0].strftime('%Y-%b-%d %H:%M:%S')])
    ...
    ax.coastlines(linewidth=0.15)
    ax.gridlines(draw_labels=True, linewidth=1, alpha=0.3, linestyle='--')
    
    #  plot trajectory
    ax.plot(lon, lat, transform = ccrs.PlateCarree(), 
            linestyle='dashed', c='k', zorder=20)
    
    # arrow linewidth
    w = 0.004;
    # plot camera look direction
    ax.quiver(lon[::step], lat[::step], vec[::step,0], vec[::step,1], 
              width=w, scale=scale_arrow, zorder = 20,
              transform = ccrs.PlateCarree())   
    
    # calculate the tilt and rotation of the camera and the new fov coverage
    for i in range(0, len(alt), step):
        fov = fov_deg/np.sin(np.deg2rad(vec_elev[i]))
        R = earth_radius_at_latitude(lat[i])
        meters_per_degree = 2 * np.pi * R / 360 
        scaling_factor = 1 / meters_per_degree
        
        look_direction = vec[i] / np.linalg.norm(vec[i])  # normalize
        look_direction_scaled = look_direction * alt[i] * 1000
        lon_shift = scaling_factor * look_direction_scaled[0] 
        lat_shift = scaling_factor * look_direction_scaled[1] 

        # Compute the tilt of the camera (angle from the vertical direction)
        tilt_rad = np.arccos(np.dot(vec[i], [0, 0, 1]))
        tilt_deg = np.degrees(tilt_rad)
        
        # Compute the rotation of the camera around the vertical axis
        rotation_rad = np.arctan2(vec[i][1], vec[i][0])
        rotation_deg = np.degrees(rotation_rad)
        
        # # Add an ellipse with the computed tilt and rotation:
        # # no color maps. just the ellipses with the boundaries.
        # ax.add_patch(mpatches.Ellipse(xy=(lon[i] + lon_shift, 
        #                                   lat[i] + lat_shift), 
        #                               width=fov, 
        #                               height = fov*(1-tilt_deg/90), 
        #                               angle = rotation_deg,
        #                               facecolor='none', edgecolor='grey',
        #                               transform=ccrs.PlateCarree()))
        
        #  store the new values to plot colored ellipses in the next loop
        new_lat.append(lat[i] + lat_shift * scale_factor_shift)
        new_lon.append(lon[i] + lon_shift * scale_factor_shift)
        lon_comp.append(lon[i])
        lat_comp.append(lat[i])
        new_fov.append(fov)
        height.append(fov*(1-tilt_deg/90))  
        angle.append(rotation_deg)                  

        ax.text(lon[i]-3, lat[i]-10, (time_array[i].strftime("%M:%S")),
        transform=ccrs.PlateCarree(), c='b',size=10, zorder=25)
   
    num_colors = 10
    cmap = cm.get_cmap('viridis', num_colors)
          
    for loni, lati, fovi, heighti, anglei in zip(new_lon, new_lat, new_fov, 
                                                 height, angle):
        ...
        # Add the patch with the assigned color
        ax.add_patch(mpatches.Ellipse(xy=(loni, lati),
                                      width=fovi,
                                      height=heighti,
                                      angle=anglei,
                                      facecolor=color,
    ...
    ax.text(lon[0]+0.5, lat[0], (time_array[0].strftime("%H:%M:%S")),
    transform=ccrs.PlateCarree(), c='b',size=10, zorder=20)

    # mark the middle of the pass
    m = int(len(alt)/2)
    ax.text(lon[m]+0.5, lat[m], (time_array[m].strftime("%H:%M:%S")),
        transform=ccrs.PlateCarree(), c='b',size=10, zorder=20)

    # mark the end of the pass
    ax.text(lon[-1]+0.5, lat[-1], (time_array[-1].strftime("%H:%M:%S")),
        transform=ccrs.PlateCarree(), c='b',size=10, zorder=20)

    # plot the location of the target
    ax.scatter(Px, Py, transform = ccrs.PlateCarree(), 
               marker = '*', s = 200, c = 'magenta', zorder=20)   
